...
public class gear_EB2 {
    ...
    public static void solve() throws Exception {
        ...
        int i = 0;
        for (; i < 4; i++) {
            Gear gear = new Gear(spl[i]);
            gearList.add(gear);
        }
        K = Integer.parseInt(spl[i]);
...
//        printGear();
        rolling(gearNum + 1, 1, clock);
        rolling(gearNum - 1, -1, clock);

//        Gear leftGear = gearList.get(gearNum-1);
//        Gear rightGear = gearList.get(gearNum+1);
    ...
    public static void rolling(int gearNum, int direction, int clock) {
        ...
        if (direction == 1) {
            if (beforeGear.magList[beforeGear.getRightIndex()] != currentGear.magList[currentGear.getLeftIndex()]) {
                ...
                currentGear.upIndex = (8 + currentGear.upIndex - clock) % 8;
                rolling(gearNum + 1, direction, clock);
            }
        } else {
            //왼쪽방향
